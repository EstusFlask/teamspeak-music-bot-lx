...
- **洛雪自定义源（可选）** — 在 **设置 → 洛雪音乐源** 在线或本地导入兼容 LX Music 的 JavaScript 音源，选择当前源并在线更新；支持为网易云（`wy`）、QQ（`tx`）、酷狗（`kg`）解析播放 URL，可配置失败后回退项目自带接口，也可把「洛雪音乐」设为默认音源。脚本在有超时/内存限制的独立 Worker 中运行，网络访问会拒绝本机与内网地址；仍应只导入可信脚本
...
2. 在 **设置 → 机器人管理** 中点击"创建新实例"，填写：
   ...
   - 机器人昵称
   - 可选：服务器密码、默认频道
3. 在 **设置 → 音乐账号** 扫码登录网易云 / QQ 音乐 / 酷狗音乐 / B 站账号；需要洛雪解析时，在同页的 **洛雪音乐源** 卡片导入并启用自定义源
...
| **设置** | 账户（修改自己密码） / 主题切换 / 机器人管理 / 行为设置 / 多平台账号登录 / 洛雪自定义源导入、选择与更新 / 默认音源 / 音质选择 / 命令前缀 / 用户管理 / 操作审计 |
...
- **自定义默认音源（`defaultPlatform`）** — 想让不带标志的 `!play 歌名` 直接用某个音源，可在 设置 → 默认音源 里选择，或在 `config.json` 中设置。普通取值须存在于 `enabledProviders`；洛雪启用后还可设为 `"lx"`，搜索元数据来自 `lxMusic.searchProvider`，播放 URL 由当前洛雪脚本解析。留空 / `null` / 删除该字段即恢复固定优先级，保存后即时生效
...
在 **设置 → 洛雪音乐源** 导入脚本后选择当前使用的源，再开启洛雪解析。在线 URL 导入的源可点击「更新」重新下载并验证；本地文件没有可靠的远程地址，需要重新导入新文件。当前兼容范围为网易云 `wy`、QQ `tx`、酷狗 `kg`，洛雪脚本只接管播放 URL，搜索、歌单和专辑仍由项目原有 Provider 提供。
...
导入的脚本和索引保存在 `data/lx-sources/`。第三方脚本虽然在独立 Worker 中运行并受超时、内存和网络地址限制，但仍应视为不受信任代码，只导入你确认可信的来源；项目不会内置或分发第三方音源脚本。
